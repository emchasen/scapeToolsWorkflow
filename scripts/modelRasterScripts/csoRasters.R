...
erosion_mod <- readRDS("cso_erosion_southEastWI.rds") #3
pi_mod <- readRDS("cso_ploss_southEastWI.rds")
ffcn_mod <- readRDS("cso_ffcn_southEastWI.rds")
sci_mod <- readRDS("cso_sci_southEastWI.rds") #4
...
level_df <- left_join(level_df, pneeds) %>%
  ...
  mutate(initialP = 45) ##TODO change intial P value for region 
...
for(i in 1:nrow(level_df)) {
  erosion.name[[i]] = paste("cornSoyOat_Erosion",
...
sci.name = list()
for(i in 1:nrow(level_df)) {
  sci.name[[i]] = paste("cornSoyOat_SCI",
                        level_df[i,10], # cover crop
                        level_df[i,11], #tillage
                        level_df[i,12], #contour
                        level_df[i,5], # manure
