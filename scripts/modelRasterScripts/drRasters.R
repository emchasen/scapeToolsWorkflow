...
erosion_mod <- readRDS("dr_erosion_southEastWI.rds") #4
#erosion_mod <- readRDS("modelOutputs/southEastWI/erosion/dr_erosion_southEastWI.rds")
pi_mod <- readRDS("dr_ploss_southEastWI.rds")
ffcn_mod <- readRDS("dr_ffcn_southEastWI.rds")
sci_mod <- readRDS("dr_sci_southEastWI.rds") #4
...
cov_till_list <- c("cc nt", "cc sc", "cc su", 
                   "gcds nt", "gcds sc", "gcds su", 
                   "gcis nt", "gcis sc", "gcis su",
                   "nc fc", "nc fm", "nc nt", "nc sv", "nc sn", "nc su")
...
level_df <- left_join(level_df, pneeds) %>%
  ...
  mutate(initialP = 45) ##TODO change initial P for region 
...
for(i in 1:nrow(level_df)) {
  erosion.name[[i]] = paste("dairyRotation_Erosion",
...
pi.name = list()
for(i in 1:nrow(level_df)) {
  pi.name[[i]] = paste("dairyRotation_PI",
                       level_df[i,10], # cover crop
                       level_df[i,11], #tillage
                       level_df[i,12], #contour
                       level_df[i,5], # manure
...
cn.name = list()
for(i in 1:nrow(level_df)) {
  cn.name[[i]] = paste("dairyRotation_CN",
                       level_df[i,10], # cover crop
                       level_df[i,11], #tillage
                       level_df[i,12], #contour
                       level_df[i,5], # manure
...
sci.name = list()
for(i in 1:nrow(level_df)) {
  sci.name[[i]] = paste("dairyRotation_SCI",
                        level_df[i,10], # cover crop
                        level_df[i,11], #tillage
                        level_df[i,12], #contour
                        level_df[i,5], # manure
...
cluster <- makeCluster(5) ##THIS IS A GUESS AT HOW MANY CORES TO RUN
