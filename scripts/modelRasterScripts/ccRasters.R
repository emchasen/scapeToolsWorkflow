...
# load models
erosion_mod <- readRDS("cc_erosion_southEastlWI.rds") # in southEast, used model version 3
#erosion_mod <- readRDS("modelOutputs/southEastlWI/erosion/cc_erosion_southEastlWI.rds")
pi_mod <- readRDS("cc_ploss_southEastlWI.rds")
ffcn_mod <- readRDS("cc_ffcn_southEastlWI.rds")
sci_mod <- readRDS("cc_sci_southEastlWI.rds")
...
level_df <- left_join(level_df, pneeds) %>%
  ...
  mutate(initialP = 45) ##TODO change initialP per region
