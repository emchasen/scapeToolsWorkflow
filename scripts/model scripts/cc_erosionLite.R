...
summary(ccFull)
print(paste('cc data is', dim(ccFull)))

# sample 1
set.seed(123)
split <- initial_split(ccFull, prop = .7, strata = Erosion)
cc <- training(split)
cc_test <- testing(split)

print(paste('new cc data is', dim(cc)))

#partition data
set.seed(123)
split2 <- initial_split(cc, strata = Erosion)
...
rf_grid <- grid_regular(
 mtry(range = c(3, 7)),
 min_n(range = c(5, 8)),
 levels = 2 #was 4, changed to see if we could speed up
)
...
metrics <- regular_res %>%
 collect_metrics()
write.csv(metrics, "cc_ErosionMetrics_southEastWI.csv", row.names = FALSE, quote = FALSE)
...
saveRDS(mod, "cc_erosion_southEastWI.rds")
...
print(paste("RMSE on cc_test", RMSE2))
