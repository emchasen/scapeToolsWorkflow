...
#partition data
set.seed(123)
split2 <- initial_split(cg, strata = SCI)
...
metrics <- regular_res %>%
  collect_metrics()
write.csv(metrics, "cg_sci_southEastmetrics.csv", row.names = FALSE, quote = FALSE)
...
saveRDS(mod, "cg_sci_southEastWI.rds")
...
print(paste("RMSE is on cgFull", RMSE))
...
print(paste("RMSE is on cg test", RMSE2))
